...
export const DEFAULT_TRACKS: PlaylistTrack[] = [
  {
    ...
    artwork_url: "https://images.unsplash.com/photo-1518609878373-06d740f60d8b?auto=format&fit=crop&w=300&q=80",
    sort_order: 1,
  },
  {
    id: "default-2",
    ...
    artwork_url: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?auto=format&fit=crop&w=300&q=80",
    sort_order: 2,
  },
];
...
export function MusicProvider({ children }: { children: ReactNode }) {
  ...
  const wantPlayingRef = useRef(false); // default paused until user toggles or plays
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (cancelled) return;
        const list = data && data.length > 0 ? (data as PlaylistTrack[]) : DEFAULT_TRACKS;
        setTracks(list);
        const saved = loadSaved();
        wantPlayingRef.current = saved.wantPlaying === true;
        if (saved.trackId) {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "playlist" }, () => {
        supabase
          ...
          .order("sort_order")
          .then(({ data }) => {
            if (data && data.length > 0) setTracks(data as PlaylistTrack[]);
          });
      })
      .subscribe();
